...
// Builds a temp-file path next to `resolvedTarget` for the atomic
// write-then-rename pattern used by `fs.writeFileAtomic`. The result has the
// form `<dir>/<base>.<suffix>.<pid>.<8-hex>` so the temp file lives on the
// same filesystem as the target and does not collide with the target on
// listings. Randomness comes from `Math.random()` and collisions are handled
// by the caller retrying with a fresh name on `EEXIST` (`wx` open flag).
function getAtomicTempPath(resolvedTarget, suffix) {
  const dir = pathModule.dirname(resolvedTarget);
  const base = pathModule.basename(resolvedTarget);
  const rand = StringPrototypePadStart(
    NumberPrototypeToString(MathFloor(MathRandom() * 0x100000000), 16),
    8, '0');
  return `${dir}${pathModule.sep}${base}.${suffix}.${process.pid}.${rand}`;
}
