...
async function writeFileAtomic(path, data, options) {
  ...
  // Resolve symlinks so the temp file lives next to the real target.
  // On ENOENT (first-ever write), resolve only the parent directory.
  let resolved;
  try {
    ...
    resolved = pathModule.join(
      await realpath(pathModule.dirname(path)),
  ...
  try {
    ...
    await handleFdClose(writeOp, fd.close);
  } catch (e) {
    await cleanup(e);
  }
  ...
  if (flush && !isWindows) {
    try {
      const dfd = await open(pathModule.dirname(resolved), 'r');
      try {
        await dfd.sync();
      } finally {
        await dfd.close();
